...
import com.netment.container.RenderWorkerExeService;
...
public class HttpRequestHandler extends SimpleChannelUpstreamHandler {
                ...
                if (!trailer.getHeaderNames().isEmpty()) {
                    for (String name : trailer.getHeaderNames()) {
                        for (String value : trailer.getHeaders(name)) {
                            logger.debug("TRAILING HEADER: " + name + " = " + value + "\r\n");
                        }
                    }
                }
                writeResponse(e, "/");
            } else {
                logger.debug("CHUNK: " + chunk.getContent().toString(CharsetUtil.UTF_8)
                        + "\r\n");
            }
        }
    }
    ...
    private void writeResponse(MessageEvent e, String uri) {
        ...
        final String path = HttpServerConfig.getRealPath(uri);
        File localFile = new File(path);
        // 如果文件隐藏或者不存在
        if (localFile.isHidden() || !localFile.exists()) {
            // 逻辑处理
            logger.info("File not found from local !! , get default page ["+HttpServerConfig.getDefaultPage()+"]");
            localFile = new File(HttpServerConfig.getDefaultPage());
        }
        // 如果请求路径为目录
        ...
        RenderWorkerExeService.getInstance().execute("index");
        try {
